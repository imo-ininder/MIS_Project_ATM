package com.attmm.admin.projectt;

/**
 * Created by imo on 2016/7/20.
 */
public interface ChatConstant {
    String CHAT_SHAREDPREFERENCES = "ATM_chatData";
    String CHAT_TITLE = "chatTitle";
    String CHAT_PATH="path";
    String CHAT_STATE="chatState";
    String CHAT_IS_DOUBLE_CHECKED = "doubleChecked";
    String CHAT_NFC_CHECK_MSG="isCompleted";
    String CHAT_IS_CANCELED="cancel";
    String AUTH_SUCCESSFUL="success";
    String CHAT_RECEIVED="com.example.admin.projectt.CHAT_RECEIVED";
    String CHAT_IS_ADMIT_CANCELED="comexampleadminprojecttCHAT_ADMIT_CANCEL";
}
